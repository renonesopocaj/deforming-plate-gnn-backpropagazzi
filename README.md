...
5. Run `python train_gunet.py` to train Graph-U-Net. See `config_gunet.yaml`
6. Run `data_exploration.py` after having generated data to explore them.
7. Run `visualize_simulation.py` to run rollout on the different models EGNN or Graph-U-Net.
8. Run `visualize_simulation_egnn.py` to run rollout on the different models EGNN or Graph-U-Net.
